...
export function useFormatDate(
  ...
  const timeToken = (() => {
    switch (activeOrg?.timeFormat) {
      case "12-hours": return "h:mm a";
      case "24-hours": return "HH:mm";
  ...
  switch (mode) {
    case "date":     return format(d, dateToken);
    case "time":     return format(d, timeToken);
    case "datetime": return format(d, `${dateToken} ${timeToken}`);
  }
}
