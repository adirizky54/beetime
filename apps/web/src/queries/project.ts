...
export const projectQueries = {
  ...
  listMembers: (orgId: string, projectId: string) =>
    queryOptions({
      ...
      queryFn: async ({ signal }) =>
        await api.get<Array<Pick<Member, "id" | "name" | "image">>>(
  ...
  create: (orgId: string) =>
    mutationOptions({
      ...
      mutationFn: async (data: CreateProjectInput) =>
        await api.post<Project>(`v1/organizations/${orgId}/projects`, data),
    }),
  ...
  update: (orgId: string, projectId: string) =>
    mutationOptions({
      ...
      mutationFn: async (data: UpdateProjectInput) =>
        await api.patch<Project>(`v1/organizations/${orgId}/projects/${projectId}`, data),
    }),
